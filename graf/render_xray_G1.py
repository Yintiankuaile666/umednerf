...
def test(range_phi, render_radius, theta_mean,
    ...
    with torch.no_grad():
        ...
        for i in range(N_samples):
            os.makedirs(eval_dir + '/generated_{:04d}'.format(iteration))
#            checkpoint_io.save('knee_model_%08d.pt' % iteration, it=iteration)
            for j in range(72):
                save_image(rgb[i][j], os.path.join(eval_dir + '/generated_{:04d}'.format(iteration),
                                                   'generated{:02d}.png'.format(j)))
...
def reconstruct(args, config_file):
    ...
    for iteration in range(5000):
        ...
        loss_1 = percept(torch.unsqueeze(xray_recons[0][0], 0), target_xray).sum()
        loss_2 = F.mse_loss(torch.unsqueeze(xray_recons[0][0], 0), target_xray)
        rec_loss = awl(loss_1, loss_2)
        rec_loss.backward()
