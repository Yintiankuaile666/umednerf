...
class Discriminator(nn.Module):
    def __init__(self, nc=3, ndf=64, imsize=64):
        ...
        self.down_from_big = nn.Sequential(
            RepVGGBlock(nc, nfc[512], 3, stride=1, padding=1, deploy=False),
            nn.LeakyReLU(0.2, inplace=True)
        )

        self.down_4 = RepVGGBlock(nfc[512], nfc[256], 3, stride=2, padding=1, deploy=False)
        self.down_8 = RepVGGBlock(nfc[256], nfc[128], 3, stride=2, padding=1, deploy=False)

        sequence = [nn.Conv2d(nfc[128], nfc[32], kernel_size=1, stride=1, padding=0, bias=False),
                    nn.BatchNorm2d(nfc[32]),
                    nn.LeakyReLU(0.2, inplace=True)]

        self.dag_heads = []
        for i in range(4):
            head = sequence + [nn.Conv2d(nfc[32], 1, kernel_size=4, stride=1, padding=0, bias=False)]
